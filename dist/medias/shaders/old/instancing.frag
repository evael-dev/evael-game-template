...
void main()
{
    ...
    vec4 result = vec4(getDirectionalLightColor(dirLight, n, viewDir), 1);
	
	for(int i = 0; i < pointsLightsNumber; i++)
	{
		if(pointsLights[i].isEnabled)
		{
			result += vec4(getPointLightColor(pointsLights[i], n, fragPos), 0);
		}
	}
	
	result += vec4(ambientLight.value, 0);

	if(textured == 1.0f)
	{
		result *= texture(myTexture, texCoord) * color;
	}
	else result *= color;
	
	outputColor = vec4(result * colorMul);
}
